    ...
    return list.map((listItem) => {
      return h(CommonCard, { title: listItem.label }, () => {
        return listItem.item.map((item) => {
          return handleControlShow(data, item.control) ? generateDynamicItem(data, listItem, item) : null
        })
      })
    })
  }
...
const generateCell = <T extends AllProperty>(item: ListItem<T>, list: List<T>, slot: () => JSX.Element) => {
  return h(
    CommonCell,
    {
      label: item.label,
...
const generateRadio = <T extends AllProperty>(data: T, item: ListItem<T> & { type: 'ElRadio' }) => {
  ...
  return h(
    ElRadioGroup,
    {
      ...
      'onUpdate:modelValue': (e: string | number | boolean) => ((data[item.prop] as string | number | boolean) = e),
    },
    {
      default: () =>
        config?.map((radio) =>
...
const generateNumber = <T extends AllProperty>(data: T, item: ListItem<T> & { type: 'ElNumber' }) => {
  const config = item.config as NumberConfig
  return h(ElInputNumber, {
    modelValue: data[item.prop] as number,
    'onUpdate:modelValue': (val: number | undefined) => ((data[item.prop] as number | undefined) = val),
    min: config?.min,
    max: config?.max,
    styleType: {
      width: 'width: 150px',
...
const generateCommonNumber = <T extends AllProperty>(data: T, item: ListItem<T> & { type: 'CNumber' }) => {
  const config = item.config as NumberConfig
